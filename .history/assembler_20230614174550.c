...
dict_t** init_opcodes() {
    ...
    addItem(opcodes, "neg", "11011");
    addItem(opcodes, "and", "11101");
    addItem(opcodes, "or", "11110");
    addItem(opcodes, "mul", "11111");
    return opcodes;
}
...
void convert(char* in, char* out) {
    ...
    while(fgets(line, sizeof(line), f_in) != NULL) {
        ...
        if(!strcmp(token, "add") || !strcmp(token, "sub") || !strcmp(token, "lbr") || !strcmp(token, "sbr")
            || !strcmp(token, "lsr") || !strcmp(token, "asr") || !strcmp(token, "lsl") || !strcmp(token, "neg")
            || !strcmp(token, "and") || !strcmp(token, "xor") || !strcmp(token, "mul")) {       // R type instructions
            char* dest = strtok(NULL, " ");
            char* d_addr = reg_to_addr(dest);
            fprintf(f_out, "%s", d_addr);
            free(d_addr);
        } else if(!strcmp(token, "lbr") || !strcmp(token, "sbr") || !strcmp(token, "addi")) {  // I type instructions
            char* rt = strtok(NULL, " ");
            char* immed = strtok(NULL, " ");
            immed[5] = '\0';
            char* rt_addr = (!reg_value(rt)) ? "0" : "1";
            fprintf(f_out, "%s%s", rt_addr, immed);
