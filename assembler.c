...
dict_t** init_opcodes() {
    ...
    addItem(opcodes, "and", "11100");
    addItem(opcodes, "xor", "11101");
    addItem(opcodes, "rxor", "11110");
    addItem(opcodes, "or", "11111");
    return opcodes;
}
...
void convert(char* in, char* out) {
    ...
    while(fgets(line, sizeof(line), f_in) != NULL) {
        ...
        if(!strcmp(token, "add") || !strcmp(token, "sub") || !strcmp(token, "lbr") || !strcmp(token, "sbr")
            ...
            || !strcmp(token, "and") || !strcmp(token, "xor") || !strcmp(token, "rxor") || !strcmp(token, "or")) {       // R type instructions
